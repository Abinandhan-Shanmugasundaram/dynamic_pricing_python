...
def train_models():
    ...
    # Convert categorical weather factor
    ...
    df["traffic_factor"] = df["traffic_factor"].fillna(df["traffic_factor"].mean())
    df["demand_supply_factor"] = df["demand_supply_factor"].fillna(df["demand_supply_factor"].mean())
    ...
    # Features and target
    X = df[["distance_km", "traffic_factor", "weather_factor", "demand_supply_factor"]]
    y = df["fare"]
    ...
    # 🚀 Scale output (fare) for models like MLP
